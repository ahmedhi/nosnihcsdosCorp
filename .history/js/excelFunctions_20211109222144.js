...
const addRowsToExistingTable = (sheet_data) => {
  ...
  for (let row = 1; row < sheet_data.length; row++) {
    ...
    for (let cell = 0; cell < nbr_col; cell++) {
      if (sheet_data[row][cell] == null) {
        tableRow[sheet_data[0][cell].trim()] = `<input value="" name="${sheet_data[0][cell]}[]">`; 
        continue;
      }
      tableRow[sheet_data[0][cell].trim()] = `<input value="${sheet_data[row][cell]}" name="${sheet_data[0][cell]}[]">`;
    }
