...
const displayHeaderData = (sheet_data) => {
  ...
  for (let cell = 0; cell < nbr_col; cell++) {
    header +=  `<th id=${sheet_data[0][cell].trim()}>${DATA_INPUT_HEADER[sheet_data[0][cell].trim()]}</th>`;
  }
  return header;
};
...
const addRowsToExistingTable = (sheet_data) => {
  ...
  const headerItems = $("#dataTable thead tr th").map(function() {
    return this.id;
  });
  ...
  for (let row = 1; row < sheet_data.length; row++) {
    ...
    for (let cell = 0; cell < nbr_col; cell++) {
      if (sheet_data[row][cell] == null) {
        tableRow[sheet_data[0][cell]] = `<input value="" name="${sheet_data[0][cell]}[]">`; 
        continue;
      }
      tableRow[sheet_data[0][cell]] = `<input value="${sheet_data[row][cell]}" name="${sheet_data[0][cell]}[]">`;
    }
