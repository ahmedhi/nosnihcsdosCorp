    ...
    for(let row = 1; row < sheet_data.length; row++) {
        ...
        for(let cell = 0; cell < nbr_col; cell++) {
            if(sheet_data[row][cell] == null){
                tableRows += `<td> <input value="" id="${sheet_data[0][cell]}_${row}" name="${sheet_data[0][cell]}"></td>`;
                continue;
            }
            tableRows += `<td> <input value="${sheet_data[row][cell]}" id="${sheet_data[0][cell]}_${row}" name="${sheet_data[0][cell]}"></td>`;
        }
        tableRows += '</tr>';
    }
    return tableRows;
...
const addRowsToExistingTable = (sheet_data) => {
    ...
    for(let row = 1; row < sheet_data.length; row++) {
        ...
        for(let cell = 0; cell < nbr_col; cell++) {
            if(sheet_data[row][cell] == null){
                tableRow.push(`<input value="" id="${sheet_data[0][cell]}_${row}" name="${sheet_data[0][cell]}">`);
                continue;
            }
            tableRow.push(`<input value="${sheet_data[row][cell]}" id="${sheet_data[0][cell]}_${row}" name="${sheet_data[0][cell]}">`);
        }
