const displayHeaderData = (sheet_data, table_output) => {
    const nbr_col = sheet_data[0].length;
    for(let cell = 0; cell < nbr_col; cell++) {
        table_output += '<th>'+sheet_data[0][cell]+'</th>';
    }
}
...
$(document).ready(function() {
    ...
    excel_file.addEventListener('change', (event) => {
        ...
        if (sheet_data.length > 0 && !doesTableExist) {
            ...
            table_output += '<thead> \n';
            displayHeaderData(sheet_data, table_output);
            table_output += '</thead> \n' +
                            '<tbody> \n' +
                            '</tbody> \n' +
                        '</table>\n ' +
