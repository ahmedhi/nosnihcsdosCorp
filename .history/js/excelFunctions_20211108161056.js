//const { sweetAlert } = require("./demo/JsAlert");
import common_consts from "./common/constants";
const {
  DATA_INPUT_HEADER,
} = common_consts;
console.log(DATA_INPUT_HEADER);
const JSalertValiderMailTrue = () => {
  swal(
    "Vérification complete",
    ", Votre vérification a été bien fait",
    "success"
  );
};
const JSalertValiderMailError = () => {
  sweetAlert(
    "Les mails non vérifiées",
    ", Veuillez confirmer la validation",
    "error"
  );
};
const JSalertSave = () => {
...
const JSalertSaveConfirmed = () => {
  Swal.fire("Bien Enregistrée", "", "success");
};
const JSalertSaveisDenied = () => {
...
const onValiderMail = () => {
  $("#btn_validerMail").click(function (e) {
    e.preventDefault();
    JSalertValiderMailTrue();
    $.ajax({
      url: "serverSide/ValidationMail.php",
      success: function (result) {
        if(result == "verification est bonne" ){ 
          JSalertValiderMailTrue();
       } else {
        JSalertValiderMailError();
       };
      },
    });
